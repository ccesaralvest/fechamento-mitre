...
interface SchedulingModalSectionProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (formData: SchedulingModalSectionFormData) => void;
}

interface SchedulingModalSectionFormData {
  name: string;
  email: string;
  fone: string;
  date?: string;
...
const SchedulingModalSection: React.FC<SchedulingModalSectionProps> = ({
  isOpen,
  onClose,
  onSubmit,
}) => {
  ...
  const processSubmit = async (data: z.infer<typeof FormSchema>) => {
    ...
    try {
      ...
      if (response) {
        ...
        } as SchedulingModalSectionFormData);
      }
    } catch (error: unknown) {
          ...
          <form onSubmit={handleSubmit(processSubmit)} className="p-6">
            <div className="space-y-4">
              ...
              <button
                type="submit"
                disabled={
                  !selectedDate || !selectedTime || !readPolicy || isLoading
                }
...
export default SchedulingModalSection;
